...
public extension MLMultiArray {
    ...
    func toIntArray() -> [Int] {
        var arr = Array(repeating: 0, count: self.count)
        let ptr = UnsafeMutablePointer<Int32>(OpaquePointer(self.dataPointer))
    ...
    func toDoubleArray() -> [Double] {
        var arr: [Double] = Array(repeating: 0, count: self.count)
...
extension MLMultiArray {
    ...
    func debug(_ indices: [Int]) -> String {
        ...
        if indices.count < shape.count { // not last dimension yet?
            for i in 0..<N {
                indices[d] = i
                s += self.debug(indices) // then call recursively again
                if i != N - 1 {
                    s += ",\n" + indent(d + 1)
                }
            }
